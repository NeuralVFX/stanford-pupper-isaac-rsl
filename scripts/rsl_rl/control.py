...
base_command = [0.0, 0.0, 0.0]  # [lin_x, lin_y, ang_z]
...
import stanford_pupper.tasks  # noqa: F401
...
def sub_keyboard_event(event, *args, **kwargs):
    ...
    if event.type == carb.input.KeyboardEventType.KEY_PRESS:
        if event.input.name in INPUT_KEYBOARD_MAPPING:
            for i, v in enumerate(INPUT_KEYBOARD_MAPPING[event.input.name]):
                base_command[i] += v
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # Hook up keyboard
    teleop = Se2Keyboard(Se2KeyboardCfg())
    teleop.reset()
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            delta_pose = teleop.advance()  # returns [lin_x, lin_y, ang_z]
